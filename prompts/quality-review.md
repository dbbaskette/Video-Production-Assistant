...
7. **Pacing** — Optional. Each narrated scene's context includes a `Narration timing:` line with the spoken length, existing pause length, the recording length, and the resulting **dead air**. When dead air is **large** (roughly ≥ 5s, or the recording is well over twice the spoken length), the narration will feel rushed against a long clip. Emit **one** suggestion with `category: "pacing"` telling the author they can add deliberate beats with the inline token — quote the exact syntax **`[pause 1.5s]`** (any duration 0.1–10s) — placed where they want the narration to breathe or sync to on-screen action. Use `severity: "info"` (or `"warn"` for very large dead air). Set `category: "pacing"` (NOT `"narration"` — this suggests *adding* pauses, the opposite of the Tighten action). Do NOT suggest pacing when dead air is small, when there is no narration audio, or when the scene already contains meaningful pauses.
