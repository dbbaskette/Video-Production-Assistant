...
5. **Narration** *(optional)* — Project-wide `/narration` page shows per-scene script + audio status and can narrate every scripted scene in one run. Choose an engine, voice, speed, and emotiveness; scenes without scripts are skipped, and existing audio is preserved unless **Overwrite existing narration** is selected. TTS produces per-paragraph MP3 chunks + SRT/VTT subtitles. Per-mode chunks survive monologue↔dialog toggling.
...
8. **Quality Review** — AI inspection of the storyboard. Stale-cache banner reminds you to re-run after major changes. Optional features (narration, scripts, lower-thirds) are not flagged when absent. The narration-length check uses your project's **measured TTS rate** (empirically averaged from already-generated chunks; 150 wpm default before any narration exists). Narration-category warnings come with a **✨ Tighten script** button that asks the LLM to *remove* content from the existing script (not rewrite), shows the proposal next to the current script, and on accept saves it + clears the stale TTS chunks.
...
| **TTS Narration** | Per-paragraph chunked TTS with multiple engines/voices, word-level timing, SRT/VTT subtitles, plus one-click project narration for all scenes that already have scripts. Empty scripts are skipped; overwrite is opt-in. Cloned voices appear automatically in the engine voice picker. |
| **Scene Preview** | Combined recording + narration + lower-thirds preview, played inline without rendering |
| **Lower Thirds** | AI-recommended title overlays with style/timing controls and ffmpeg rendering. Project-wide `/lower-thirds` page shows counts + first titles + time ranges per scene. The scene-level editor includes a **drag-to-position timeline scrubber** above the numeric in/out cards — each LT is a band over a track sized to the recording duration; drag the middle to shift, drag the edges to resize. Save / Recommend invalidates the baked overlay + framed video on disk, so the next render reflects exactly what's on the LT tab — including "I just deleted every entry, save the empty state." |
| **Scene Transitions** | Per-scene out-transitions: cut (default), crossfade, fade-black, fade-white, wipe-left/right, slide-left/right/up/down, circleopen/close, radial, pixelize. Configurable duration 0.1–5s. **Freeze-frame transition clips** are inserted between the two source scenes — the last frame of A and first frame of B are extracted and ffmpeg `xfade`-ed across the configured duration, then joined with hard cuts. The source clips play in full; the transition is additive time. Bumpers always come in/out on hard cuts regardless of the adjacent scene's transition setting. A **▶ Preview transition** button on the Recording tab builds just the transition clip (cached at `renders/.transition-previews/`) and plays it inline so you can iterate on style + duration in ~1.5s instead of waiting for a full project render. |
...
| **Quality Review** | AI inspection of the full storyboard with severity-graded issue punch list. Optional features (narration, scripts, lower-thirds) are not flagged when absent. Stale-cache banner with one-click re-run when the project has changed since the last review. The narration-length verdict (within target / TOO LONG / unusually short) is computed server-side from your project's **measured TTS rate** — empirically averaged across every generated chunk that has a duration, falling back to 150 wpm only before any narration exists. The same rate drives the Tighten action below, so the two never disagree. |
